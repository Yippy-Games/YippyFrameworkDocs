...
local PartUtils = require(ReplicatedFirst.Framework.BuiltIn.Shared.Utils.Part)
...
function Registry:RegisterTypesInCustom(ContainerArray: table)
	for _, TypeData in pairs(ContainerArray) do
		local Object = PartUtils:findInstanceByPath(TypeData.StartInstance, TypeData.Path)
		if Object then
			Object.Parent = self.Cmdr.ReplicatedRoot.Types
			require(Object)(self)
		end
...
function Registry:RegisterCommmandsInCustom(ConfigArray: table, FuncArray: table)
	for Name, ConfigData in pairs(ConfigArray) do
		if not FuncArray[Name] then
			continue
		end
		local FuncData = FuncArray[Name]
		local ModuleConfig = PartUtils:findInstanceByPath(ConfigData.StartInstance, ConfigData.Path)
		local ModuleFunc = PartUtils:findInstanceByNameInChild(FuncData.StartInstance, FuncData.Path)
		if ModuleConfig and ModuleFunc then
			self:RegisterCommand(ModuleConfig, ModuleFunc)
		end
	end
end
...
Registry.RegisterHooksInCustom = Registry.RegisterTypesInCustom
