...
function NumberFormatterAPITest.NotationScientific()
	...
	NumberFormatterAPITest.assert_format_descending(
		...
		FormatNumber.NumberFormatter
			.with()
			:Notation(FormatNumber.Notation.scientific():WithExponentSignDisplay(FormatNumber.SignDisplay.ALWAYS)),
		"8.765E+4",
		"8.765E+3",
		"8.765E+2",
		"8.765E+1",
...
function NumberFormatterAPITest.RoundingFigures()
	...
	NumberFormatterAPITest.assert_format_single(
		...
		FormatNumber.NumberFormatter
			.with()
			:Precision(FormatNumber.Precision.fixedSignificantDigits(1))
...
function NumberFormatterAPITest.RoundingFractionFigures()
	...
	NumberFormatterAPITest.assert_format_descending(
		...
		"precision-integer/@#r",
		"./@#r",
		FormatNumber.NumberFormatter.with():Precision(
			FormatNumber.Precision.maxFraction(0):WithSignificantDigits(1, 2, FormatNumber.RoundingPriority.RELAXED)
		),
		"87,650",
		"8,765",
		"876",
	...
	NumberFormatterAPITest.assert_format_descending(
		...
		"./@#s",
		FormatNumber.NumberFormatter.with():Precision(
			FormatNumber.Precision.maxFraction(0):WithSignificantDigits(1, 2, FormatNumber.RoundingPriority.STRICT)
		),
		"88,000",
		"8,800",
		"880",
	...
	NumberFormatterAPITest.assert_format_single(
		...
		".0/@@@r",
		FormatNumber.NumberFormatter.with():Precision(
			FormatNumber.Precision.fixedFraction(1):WithSignificantDigits(3, 3, FormatNumber.RoundingPriority.RELAXED)
		),
		1,
		"1.00"
	)
	...
	NumberFormatterAPITest.assert_format_single(
		...
		".0/@@@s",
		FormatNumber.NumberFormatter.with():Precision(
			FormatNumber.Precision.fixedFraction(1):WithSignificantDigits(3, 3, FormatNumber.RoundingPriority.STRICT)
		),
		1,
		"1.0"
	)
	...
	NumberFormatterAPITest.assert_format_single(
		...
		"./@@@s",
		FormatNumber.NumberFormatter.with():Precision(
			FormatNumber.Precision.fixedFraction(0):WithSignificantDigits(3, 3, FormatNumber.RoundingPriority.STRICT)
		),
		9.99,
		"10"
	)
...
function NumberFormatterAPITest.RoundingOther()
	...
	NumberFormatterAPITest.assert_format_descending(
		...
		FormatNumber.NumberFormatter
			...
			:RoundingMode(FormatNumber.RoundingMode.CEILING),
		"87,650",
		"8,765",
		"877",
		"88",
...
function NumberFormatterAPITest.IntegerWidth()
	...
	NumberFormatterAPITest.assert_format_descending(
		...
		"integer-width-trunc",
		"integer-width-trunc",
		FormatNumber.NumberFormatter.with():IntegerWidth(FormatNumber.IntegerWidth.zeroFillTo(0):TruncateAt(0)),
		"0",
		"0",
		".5",
		".65",
...
function NumberFormatterAPITest.SignNearZero()
	...
	for _, cas in cases do
		...
		local actual =
			FormatNumber.NumberFormatter.with():Sign(sign):Precision(FormatNumber.Precision.integer()):Format(input)
		assert(expected == actual, input .. " @ SignDisplay " .. sign)
	end
end
...
function NumberFormatterAPITest.SignCoverage()
	...
	for _, cas in cases do
		...
		for i, input in inputs do
			...
			local actual = FormatNumber.NumberFormatter.with():Sign(sign):Format(input)
			assert(expected == actual, input .. " " .. sign)
		end
	end
end
...
function NumberFormatterAPITest.assert_format_descending(
	...
	if skeleton and concise_skeleton then -- if nil, skeleton is declared as undefined.
		...
		local normalized = select(
			2,
			(select(2, FormatNumber.NumberFormatter.forSkeleton(skeleton)) :: FormatNumber.NumberFormatter):ToSkeleton()
		)
		...
		if shouldRoundTrip then
			assert_equals(message .. ": Concise Skeleton:", normalized, select(2, f3:ToSkeleton()))
		end
		for i, d in inputs do
...
function NumberFormatterAPITest.assert_undefined_skeleton(f: FormatNumber.NumberFormatter)
	...
	assert(not supported, "Expect toSkeleton to fail, but passed, producing: " .. skeleton)
end
