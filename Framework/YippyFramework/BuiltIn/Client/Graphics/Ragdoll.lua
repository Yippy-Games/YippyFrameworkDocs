...
function Ragdoll:Start()
    Ragdoll.RagdollNetwork = Framework.BuiltInClient.Network.Channel("Ragdoll")

    Ragdoll.RagdollNetwork:On("Ragdoll", function(enabled)
        if enabled then
            Ragdoll:RagdollCharacterServer()
        else
            Ragdoll:UnragdollCharacterServer()
        end
    end)
end
...
function Ragdoll:RagdollCharacterServer()
    ...
    if Humanoid.Health > 0 then
        ...
        Camera.CameraSubject = Character:FindFirstChild("UpperTorso")
...
function Ragdoll:UnragdollCharacterServer()
    local Character = Client.Character
    if not Character then
        return
    end
...
function Ragdoll:BuildRagdoll(Character: Model)
    if not Character then
        return
    end
